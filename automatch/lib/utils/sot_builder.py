...
from automatch.lib.models.sot.siaminference import SiamInference
...
class Siamese_builder(nn.Module):
    ...
    def build_backbone(self, backbone_type):
        if 'ResNet50_cross' in backbone_type:
            bk_module = importlib.import_module('automatch.lib.models.backbone.ResNet_cross')
            ...
            bk_module = importlib.import_module('automatch.lib.models.backbone.ResNet')
            bk_func = getattr(bk_module, backbone_type)
            ...
            bk_module = importlib.import_module('automatch.lib.models.backbone.Swin')
            bk_func = getattr(bk_module, backbone_type)
            backbone = bk_func(self.cfg)
        elif 'SPOS_VLT' in backbone_type:
            bk_module = importlib.import_module('automatch.lib.models.backbone.SPOS_VLT')
            bk_func = getattr(bk_module, backbone_type)
    ...
    def build_neck(self, neck_type):
        ...
        if 'Trans_Fuse' in neck_type:
            neck_module = importlib.import_module('automatch.lib.models.sot.InMo.'+self.cfg.MODEL.NECK.MODULE)  # trans_fuse/trans_fuse_win
            neck_func = getattr(neck_module, neck_type)
            neck = neck_func(self.cfg.MODEL)
        else:
            neck_module = importlib.import_module('automatch.lib.models.sot.neck')
            neck_func = getattr(neck_module, neck_type)
    ...
    def build_head(self, head_type):
        head_module = importlib.import_module('automatch.lib.models.sot.head')
