...
class RUODseq(BaseVideoDataset):
    def __init__(self, root=None, image_loader=jpeg4py_loader, data_fraction=None, split="train"):
        ...
        super().__init__('RUOD', root, image_loader)

        self.img_pth = os.path.join(root, 'images/{}/'.format(split))
        self.anno_path = os.path.join(root, 'annotations/instances_{}.json'.format(split))
