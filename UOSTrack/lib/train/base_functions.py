...
from lib.train.dataset import Lasot_lmdb, Got10k_lmdb, MSCOCOSeq_lmdb, ImagenetVID_lmdb, TrackingNet_lmdb, FishExtand, \
    Utb180, RUODseq
...
def update_settings(settings, cfg):
    ...
    # settings.warmup = cfg.TRAIN.SCHEDULER.WARMUP
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val",
                        "COCO17", "VID", "TRACKINGNET", 'FishExtand', 'UTB180', 'RUOD',]
        if name == "LASOT":
        ...
        if name == 'FishExtand':
            datasets.append(FishExtand(settings.env.fishextand_dir, image_loader=image_loader))

        if name == 'UTB180':
            datasets.append(Utb180(settings.env.utb_dir, image_loader=image_loader))

        if name == 'RUOD':
            datasets.append(RUODseq(settings.env.ruod_dir, split='train', image_loader=image_loader))

        if name == 'UOT100':
            datasets.append(UOT100(settings.env.uot_dir, image_loader=image_loader))
...
def build_dataloaders(cfg, settings):
    ...
    transform_train = tfm.Transform(
        # tfm.ToColorShuffle(probability=0.2),
        # tfm.ToBlur(probability=0.2),
        # tfm.ToGridMask(probability=0.2),
...
def get_optimizer_scheduler(net, cfg, loader_train):
    train_cls = getattr(cfg.TRAIN, "TRAIN_CLS", False)
    if train_cls:
    ...
    # if getattr(cfg.TRAIN.SCHEDULER, "WARMUP", False):
    #     lr_scheduler = WarmUpScheduler(optimizer,
    #                                    lr_scheduler,
    #                                    len_loader=len(loader_train),
    #                                    warmup_steps=cfg.TRAIN.SCHEDULER.WARMUP_STEP * len(loader_train),
    #                                    warmup_start_lr=cfg.TRAIN.SCHEDULER.WARMUP_START_LR,
    #                                    warmup_mode='linear')
...
if __name__ == '__main__':
    ...
    root = r'D:\BaiduNetdiskDownload\fish\Fishseq\fishextand'

    dataset = FishExtand(root)
