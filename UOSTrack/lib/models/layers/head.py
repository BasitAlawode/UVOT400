...
class CenterPredictor_conf(nn.Module, ):
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False):
        super(CenterPredictor_conf, self).__init__()
        self.feat_sz = feat_sz
        self.stride = stride
        self.img_sz = self.feat_sz * self.stride
        ...
        #  conf regress
        self.conv1_conf = conv(inplanes, channel, freeze_bn=freeze_bn)
        self.conv2_conf = conv(channel, channel // 2, freeze_bn=freeze_bn)
        self.conv3_conf = conv(channel // 2, channel // 4, freeze_bn=freeze_bn)
        self.conv4_conf = conv(channel // 4, channel // 8, freeze_bn=freeze_bn)
        self.conv5_conf = nn.Conv2d(channel // 8, 1, kernel_size=1)
    ...
    def get_score_map(self, x):
        ...
        score_map_conf = self.conv5_conf(x_conf4)
        return _sigmoid(score_map_ctr), _sigmoid(score_map_size), score_map_offset, score_map_conf
...
def build_box_head(cfg, hidden_dim):
    ...
    if cfg.MODEL.HEAD.TYPE == "MLP":
        ...
        center_head = CenterPredictor(inplanes=in_channel, channel=out_channel,
                                      feat_sz=feat_sz, stride=stride)
        return center_head
    elif cfg.MODEL.HEAD.TYPE == "CENTER_CONF":
        in_channel = hidden_dim
        out_channel = cfg.MODEL.HEAD.NUM_CHANNELS
        feat_sz = int(cfg.DATA.SEARCH.SIZE / stride)
        center_head = CenterPredictor_conf(inplanes=in_channel, channel=out_channel,
                                      feat_sz=feat_sz, stride=stride)
        return center_head
    else:
