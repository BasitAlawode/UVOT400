...
class BaseBackbone(nn.Module):
    ...
    def forward_features(self, z, x):
        ...
        print(r'base_backbone 1', x.shape, z.shape)
        ...
        z += self.pos_embed_z
        x += self.pos_embed_x
        print(r'base_backbone 2', x.shape, z.shape)
        if self.add_sep_seg:
            print('add_sep_seq')
        ...
        print(r'base_backbone 4', x)

        if self.add_cls_token:
            x = torch.cat([cls_tokens, x], dim=1)

        x = self.pos_drop(x)
        print(r'base_backbone 5', x)
        for i, blk in enumerate(self.blocks):
            x = blk(x)
            print(fr'base_backbone 6 {i}', x.shape)

        lens_z = self.pos_embed_z.shape[1]
        lens_x = self.pos_embed_x.shape[1]
        print(r'base_backbone 7', lens_z, lens_x)

        x = recover_tokens(x, lens_z, lens_x, mode=self.cat_mode)
        print(r'base_backbone 8',x.shape)
        aux_dict = {"attn": None}
        raise
